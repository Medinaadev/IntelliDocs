...
export class QueueService implements OnModuleDestroy {
    ...
    constructor(
        ...
        this.maxRetries =
            config.queue?.maxRetries ?? PG_PUBSUB_QUEUE_MAX_RETRIES;
        this.messageTTL =
            config.queue?.messageTTL ?? PG_PUBSUB_QUEUE_MESSAGE_TTL;
        this.cleanupInterval =
            config.queue?.cleanupInterval ?? PG_PUBSUB_QUEUE_CLEANUP_INTERVAL;
    ...
    async fetchPendingMessages<T>(
        channel: string,
    ): Promise<QueuedMessage<T>[]> {
        try {
    ...
    async markAsFailed(messageIds: number[]): Promise<void> {
        ...
        `,
            ...
            this.logger.error(`Failed to mark messages as failed:`, error);
            throw error;
        }
    }
    ...
    private async createQueueTable(): Promise<void> {
      ...
      `);
            this.logger.log(
                `Queue table "${this.queueSchema}"."${this.queueTable}" ready`,
            );
        } catch (error) {
            this.logger.error('Failed to create queue table:', error);
            throw error;
        }
    }
    ...
    private async cleanupOldMessages(): Promise<void> {
        ...
        `,
            ...
            this.logger.error('Failed to clean up old messages:', error);
            throw error;
        }
    }
}
